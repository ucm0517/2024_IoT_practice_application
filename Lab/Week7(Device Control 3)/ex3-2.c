...
#define BUTTON_ENTER_PIN 24  // E 버튼 (메뉴 선택)
...
void processPasswordInput(char key) {
    if (inputIndex < 4) {
        ...
        Change_FREQ(tones[inputIndex % 4]);  // 4개의 음을 순환하며 재생
        delay(100);  // 음 재생 시간
        STOP_FREQ();
    }
}
...
void executeMenuOption() {
    ...
    switch (currentMenu) {
        ...
        case CHANGE_PW:
            ...
            delay(500);
            inputIndex = 0;  // Change PW 메뉴 진입 시 입력 초기화
            memset(inputPassword, 0, sizeof(inputPassword));
            while (inputIndex < 4) {
                char key = readKeypad();
                if (key >= '0' && key <= '9') {
